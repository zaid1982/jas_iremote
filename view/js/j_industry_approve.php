...
<script type="text/javascript">  
    ...
    var data_ita_new;
    var data_ita_history;
    
    $(document).ready(function () {
        
        pageSetUp();
               
        $('#ita_dateReceived').daterangepicker({
            "showDropdowns": true,
            locale: {
                cancelLabel: 'Clear',
                format: 'YYYY-MM-DD'
        ...
        $('#ita_dateDue').daterangepicker({
            ...
            var filteredData = dataNew.column(8).data().filter(function (value, index) {
                var evalDate = 0;
                if (value !== null && value !== "") {
                    var dateArray = data_ita_new[index].wfTask_dateExpired.substr(0,10).split("-");
                    evalDate = dateArray[0] + dateArray[1] + dateArray[2];
                }
                if ((isNaN(dateStart) && isNaN(dateEnd)) || (evalDate >= dateStart && evalDate <= dateEnd)) {
                    return true;
        ...
        var datatable_ita = undefined;  var cnt_ita = 1;
        dataNew = $('#datatable_ita').DataTable({
            "aaSorting": [[7,'desc']],
            ...
            "preDrawCallback": function () {
                if (!datatable_ita) {
                    datatable_ita = new ResponsiveDatatablesHelper($('#datatable_ita'), breakpointDefinition);
                }
            },
            "rowCallback": function (nRow, aData, index) {
                datatable_ita.createExpandIcon(nRow);
                var info = dataNew.page.info();
                $('td', nRow).eq(0).html(info.page * info.length + (index + 1));
            },
            "drawCallback": function (oSettings) {
                datatable_ita.respond();
            },
            "oTableTools": {
                "aButtons": [
                    {
                        ...
                        "fnCellRender": function ( sValue, iColumn, nTr, iDataIndex ) {
                            if (data_ita_new.length < cnt_ita)
                                cnt_ita = 1;
                            if ( iColumn === 0 )
                                return cnt_ita++;
                            else if ( iColumn === 8 )
                                return data_ita_new[iDataIndex].wfTask_dateExpired;
                            else if ( iColumn === 9 )
                                return data_ita_new[iDataIndex].status_desc;
                            else if ( iColumn === 10 )
                                return '';
                            return sValue;
                        }
        ...
        $('#datatable_ita').on('draw.dt', function () {
            $('[data-toggle="popover"]').popover();
        });
        $("#datatable_ita thead th input[type=text]").on('keyup change', function () {
            dataNew.column($(this).parent().index() + ':visible').search(this.value).draw();
        });
        $("#datatable_ita thead th input[type=number]").on('keyup change', function () {
            dataNew.column($(this).parent().index() + ':visible').search('^'+this.value+'$', true, false, true).draw();
        });
        $("#datatable_ita thead th select").on('change', function () {
            if (this.value == '')
                dataNew.column($(this).parent().index() + ':visible').search(this.value).draw();
            else
                dataNew.column($(this).parent().index() + ':visible').search('^'+this.value+'$', true, false, true).draw();
        ...
        $('#ita2_dateSubmit').daterangepicker({
            "showDropdowns": true,
            locale: {
                cancelLabel: 'Clear',
                format: 'YYYY-MM-DD'
        ...
        $('#ita2_dateDue').daterangepicker({
            ...
            var filteredData = dataHistory.column(8).data().filter(function (value, index) {
                var evalDate = 0;
                if (value !== null && value !== "") {
                    var dateArray = data_ita_history[index].wfTask_dateExpired.substr(0,10).split("-");
                    evalDate = dateArray[0] + dateArray[1] + dateArray[2];
                }
                if ((isNaN(dateStart) && isNaN(dateEnd)) || (evalDate >= dateStart && evalDate <= dateEnd)) {
                    return true;
        ...
        var datatable_ita2 = undefined;  var cnt_ita2 = 1;
        dataHistory = $('#datatable_ita2').DataTable({
            "aaSorting": [[7,'desc']],
            "sDom": "<'dt-toolbar'<'col-sm-5 hidden-xs'l><'col-sm-7 col-xs-12'CT>r>" + "t" +
                    "<'dt-toolbar-footer'<'col-xs-5'i><'col-xs-7'p>>",
            "autoWidth": true,
            "preDrawCallback": function () {
                if (!datatable_ita2) {
                    datatable_ita2 = new ResponsiveDatatablesHelper($('#datatable_ita2'), breakpointDefinition);
                }
            },
            "rowCallback": function (nRow, aData, index) {
                datatable_ita2.createExpandIcon(nRow);
                var info = dataHistory.page.info();
                $('td', nRow).eq(0).html(info.page * info.length + (index + 1));
            },
            "drawCallback": function (oSettings) {
                datatable_ita2.respond();
            },
            "oTableTools": {
                "aButtons": [
                    {
                        ...
                        "sPdfSize": "letter",
                        "fnCellRender": function ( sValue, iColumn, nTr, iDataIndex ) {
                            if (data_ita_history.length < cnt_ita2)
                                cnt_ita2 = 1;
                            if ( iColumn === 0 )
                                return cnt_ita2++;
                            else if ( iColumn === 8 )
                                return data_ita_history[iDataIndex].wfTask_dateExpired;
                            else if ( iColumn === 9 )
                                return data_ita_history[iDataIndex].status_desc;
                            else if ( iColumn === 10 )
                                return '';
                            return sValue;
                        }
        ...
        $('#datatable_ita2').on('draw.dt', function () {
            $('[data-toggle="popover"]').popover();
        });
        $("#datatable_ita2 thead th input[type=text]").on('keyup change', function () {
            dataHistory.column($(this).parent().index() + ':visible').search(this.value).draw();
        });
        $("#datatable_ita2 thead th input[type=number]").on('keyup change', function () {
            dataHistory.column($(this).parent().index() + ':visible').search('^'+this.value+'$', true, false, true).draw();
        });
        $("#datatable_ita2 thead th select").on('change', function () {
            if (this.value == '')
                dataHistory.column($(this).parent().index() + ':visible').search(this.value).draw();
            else
                dataHistory.column($(this).parent().index() + ':visible').search('^'+this.value+'$', true, false, true).draw();
    ...
    function f_table_ita_new () {
        data_ita_new = f_get_general_info_multiple('dt_task_industrial', {wfTaskType_id:'(35,45)', wfTask_partition:'1'});
        f_dataTable_draw(dataNew, data_ita_new, 'datatable_ita', 11);
    }
    
    function f_table_ita_history () {
        data_ita_history = f_get_general_info_multiple('dt_task_industrial', {wfTaskType_id:'(35,45)', wfTask_partition:'2'});
        f_dataTable_draw(dataHistory, data_ita_history, 'datatable_ita2', 11);
    }
    
    function f_table_ita_title () {
        var txt_title = '';
        if ($('#ita_dateReceived').val() != '') {
            txt_title = 'Received Date : '+$('#ita_dateReceived').val();
            if ($('#ita_dateDue').val() != '') 
                txt_title += ', Due Date : '+$('#ita_dateDue').val();
            txt_title = '<i>('+txt_title+')</i>';
        } else {
            if ($('#ita_dateDue').val() != '') 
                txt_title += '<i>(Due Date : '+$('#ita_dateDue').val()+')</i>';
        }
        $('#ita_table_title').html(txt_title);
    }
    
    function f_table_ita2_title () {
        var txt_title = '';
        if ($('#ita2_dateSubmit').val() != '') {
            txt_title = 'Received Date : '+$('#ita2_dateSubmit').val();
            if ($('#ita2_dateDue').val() != '') 
                txt_title += ', Due Date : '+$('#ita2_dateDue').val();
            txt_title = '<i>('+txt_title+')</i>';
        } else {
            if ($('#ita2_dateDue').val() != '') 
                txt_title += '<i>(Due Date : '+$('#ita2_dateDue').val()+')</i>';
        }
        $('#ita2_table_title').html(txt_title);
    }
